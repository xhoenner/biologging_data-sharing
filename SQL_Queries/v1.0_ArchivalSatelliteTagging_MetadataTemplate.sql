...
DROP SCHEMA biologging CASCADE;
...
ALTER TABLE datacenter 	ADD CONSTRAINT datacenter_fkey_manager FOREIGN KEY (manager_id)
      ...
      ON UPDATE NO ACTION ON DELETE NO ACTION;
ALTER TABLE project ADD CONSTRAINT project_fkey_pi FOREIGN KEY (pi_id)
      REFERENCES users (id) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION;

CREATE TABLE device
(
  id character varying(255) NOT NULL, -- tag ID
  ...
  tdr boolean NOT NULL,
    CONSTRAINT device_pkey PRIMARY KEY (id, project_id),
      CONSTRAINT device_key UNIQUE (id),
...
(
  ...
  life_stage character varying(255) NOT NULL, -- e.g. adult, juvenile, subadult, weaner
  ...
  CONSTRAINT animal_life_stage_types CHECK (life_stage = 'Adult' OR life_stage = 'Juvenile' OR life_stage = 'Subadult'),
...
(
  id bigint NOT NULL,
  device_id character varying(255) NOT NULL, -- FK to device.id
  ...
  other_samples_taken character varying(255), -- Describe whether any samples were taken during the tagging experiment (e.g. DNA, blubber, blood)
    CONSTRAINT tag_deployment_pkey PRIMARY KEY (id, device_id, animal_id),
    ...
    CONSTRAINT tag_deployment_fkey_animal FOREIGN KEY (animal_id)
      ...
      ON UPDATE NO ACTION ON DELETE NO ACTION,
    CONSTRAINT tag_deployment_key UNIQUE (id)
);
